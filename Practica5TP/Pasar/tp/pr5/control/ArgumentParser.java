...
public class ArgumentParser {
	...
	public ArgumentParser(String[] args){
		...
		options.addOption("n", "numMovs", true, "Límite de movimientos.");
		...
		options.getOption("n").setArgName("limite");
	...
	public int pedirLimite() throws ArgumentException{
		if (cmd.hasOption("n")){
			try{
				return Integer.parseInt(cmd.getOptionValue("n"));
			}catch (NumberFormatException e){
				throw new ArgumentException("Argumento n incorrecto: " + cmd.getOptionValue("n"));
			}
		}
		else {
			return 0;
